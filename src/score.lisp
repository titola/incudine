...
(defmacro maybe-extend-time (now time-var dur sched-func c-array)
  ``(at (+ (rt-eval (:return-value-p t) (incudine.edf:last-time))
        ...
        (lambda ()
          (when (and (plusp ,,dur)
            ...
            (,,sched-func ,,time-var (lambda (x) (declare (ignore x)))))
          (if (rt-thread-p)
              (nrt-funcall (lambda () (free ,,c-array)))
              (free ,,c-array)))))
      ...
      (with-complex-gensyms (beats last-time last-dur c-array-wrap c-array)
        `(,@(if fname `(defun ,fname) '(lambda)) ()
           (let* ((,tempo-env (make-tempo-envelope '(60 60) '(0)))
             ...
             (symbol-macrolet ((,time (smp-ref ,c-array 0))
                               (,sched (smp-ref ,c-array 1))
                               (,last-time (smp-ref ,c-array 2))
                               (,last-dur (smp-ref ,c-array 3)))
               (setf ,time (now))
               (flet ((,dur (,beats)
                 ...
                 (macrolet ((,tempo (&rest args)
                   ...
                   (prog*
                     ,@(with-open-file (score path)
                         (append
                           (find-score-local-bindings score sched)
                           (loop for line of-type (or string null)
                                          = (read-score-line score)
                                 while line
                                 unless (score-skip-line-p line)
                                 collect (score-line->sexp line sched))
                           (list (maybe-extend-time sched last-time last-dur
                                   %sched c-array-wrap))))))))))))))
