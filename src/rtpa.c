...
#define __PA_RUNNING     (0)
#define __PA_STOPPED     (1)
...
static int pa_status = __PA_STOPPED;
...
unsigned int pa_cycle_begin()
{
    if (pa_status != __PA_RUNNING)
        return 0;
...
#define __pa_condition_wait(COND,LOCK)  \
    pthread_mutex_lock(LOCK);           \
    pthread_cond_wait(COND, LOCK);      \
    pthread_mutex_unlock(LOCK);

#define __pa_condition_signal(COND,LOCK)  \
    pthread_mutex_lock(LOCK);             \
...
void *pa_process_thread(void *arg)
{
    ...
    while(1) {
        ...
        if (pa_lisp_busy != 0) {
             ...
             * Notice it is called ONLY ONE TIME after the first cycle and
             * ONLY ONE TIME after the gc in SBCL. Practically, this thread
             * is an emergency exit when we use an implementation of Common
             * Lisp with a gc which stops the rt lisp thread. If the
             * implementation of CL has a realtime gc, there aren't other
             * transfers of the control from C to Lisp and vice versa.
             */
                  ...
                  const char* client_name)
{
    ...
    err = Pa_OpenStream(&stream, iparam, oparam, (double) srate,
...
int pa_start()
{
    ...
    /*
     * Auxiliary C realtime thread. If lisp is busy, it continues the
     * work to avoid xruns.
     */
    if ((err = pthread_create(&process_thread, NULL, pa_process_thread, NULL))) {
