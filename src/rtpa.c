...
int pa_cache_inputs(void)
{
	...
	n = 0;
	if (pa_inputs_cache != NULL) {
		n = incudine_ringbuffer_write(pa_inputs_cache, (char *) pa_inputs,
		                              pa_inbuf_bytes);
	}
...
int pa_inputs_from_cache_begin(void)
{
	...
	if (pa_inputs_cache != NULL) {
		...
		n = incudine_ringbuffer_read(pa_inputs_cache, (char *) pa_tmp_inputs,
		                             pa_inbuf_bytes);
		tmp = lisp_input;
	...
	while(pa_status == PA_RUNNING) {
		...
		if (lisp_busy) {
			...
			if (n < frames_per_buffer) {
				/*
				 * Ensure blocking, otherwise the recovery in Lisp-thread is
				 * possibly slower than the suspended cycles in C-thread.
			...
			if (pa_inputs_cache != NULL) {
				incudine_ringbuffer_write(pa_inputs_cache, (char *) pa_inputs,
				                          pa_inbuf_bytes);
			}
			MAYBE_CONTINUE_IN_LISP(pa_lisp_busy, resume_lisp);
		...
		resume_lisp:
			...
			if ((cycles > 0) && (!(pa_has_cached_inputs())))
                                ...
                                unsigned long nframes, double srate)
{
	if (value == 0.0) {
		...
		return (PaTime) ((-value) * (nframes / srate));
	}
	return value;
}
...
double pa_get_stream_latency(int is_input)
{
	if (stream != NULL) {
		...
		if (info != NULL)
			return (double) (is_input ?
			                 info->inputLatency :
			                 info->outputLatency);
	}
	return (is_input ? pa_input_latency : pa_output_latency);
}

void pa_set_stream_latency(double value, int is_input)
{
	if (is_input)
		pa_input_latency = (PaTime) value;
	else
		pa_output_latency = (PaTime) value;
}
                  ...
                  SAMPLE *sample_counter)
{
	...
	if (input_channels == 0 && output_channels == 0) {
		pa_set_error_msg("Zero input channels and zero output channels.");
		return 1;
	}
	err = Pa_Initialize();
	if (err != paNoError) {
	...
	if (input_channels > 0) {
		...
		if (input_param.device != paNoDevice) {
			...
			input_param.suggestedLatency =
				pa_update_latency(pa_input_latency,
	...
	if (output_channels > 0) {
		...
		if (output_param.device != paNoDevice) {
			...
			output_param.suggestedLatency =
				pa_update_latency(pa_output_latency,
	...
	if (input_channels > 0) {
		...
		RETURN_IF_NULLPTR(pa_tmp_inputs, "malloc of temporary inputs failed");
	}
...
double pa_get_time_offset_seconds(void)
{
	if (stream != NULL) {
		...
		time_from_cycle_start = Pa_GetStreamTime(stream) - pa_cycle_start_time_sec;
		frames_to_seconds =
