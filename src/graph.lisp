...
(defstruct (node (:include incudine-object)
  ...
  ;; is a group, it is the FUNCONS of the next node of that group.
...
(defun paused-parent (obj)
  (declare (type node obj) #.*standard-optimize-settings*)
  (loop for n = (node-parent obj) then (node-parent n)
        while (node-p n)
        if (node-pause-p n) return n))

(defun unpaused-node-prev (obj)
  (declare (type node obj) #.*standard-optimize-settings*)
  (loop for n = obj then prev
        for prev = (node-prev n)
        while (node-p prev)
        unless (node-pause-p prev)
        do (let ((paused-prev (paused-parent prev)))
             (if paused-prev
                 (setf prev paused-prev)
                 (return prev)))))

(defun unpaused-node-next (obj &optional root-p)
  (declare (type node obj) #.*standard-optimize-settings*)
  (flet ((current-node (x)
           (declare (type node x))
           (if (node-pause-p x)
               (the node (find-last-node x))
               x)))
    (loop for n = (if root-p obj (current-node obj))
                then (current-node next)
          for next = (node-next n)
          while (node-p next)
          unless (node-pause-p next)
          do (let ((paused-next (paused-parent next)))
               (if paused-next
                   (setf next paused-next)
                   (return next))))))

(declaim (inline paused-node-p))
(defun paused-node-p (obj)
  (or (node-pause-p obj) (paused-parent obj)))

(defmacro update-funcons (node next-funcons &key group-body group-not-body)
  (with-gensyms (n f)
    `(let ((,n ,node)
           (,f ,next-funcons))
       (cond ((group-p ,n)
              (setf (node-funcons ,n) ,f)
              ,@group-body)
             (t (setf (cdr (node-funcons ,n)) ,f)
                ,@group-not-body)))))

(defun update-previous-funcons (node &optional pause-p end-node)
  (declare (type node node))
  (flet ((prev (n pause-p)
           (if pause-p
               (when (and (paused-node-p (node-prev n))
                          (not (node-root-p (node-prev n))))
                 (node-prev n))
               (unpaused-node-prev n))))
    (loop for obj = node then prev
          for pause = pause-p then (paused-node-p obj)
          for prev = (prev obj pause)
          while (node-p prev)
          do (update-funcons prev (node-funcons obj)
               :group-body
                 ((when (and end-node (eq prev end-node)) (return)))))))

(defmacro update-next-funcons (node-var group-p pause-p)
  (let* ((f `(node-funcons ,node-var))
         (f (if group-p f `(cdr ,f)))
         (next (gensym)))
    `(if ,pause-p
         (setf ,f (node-funcons (node-next ,node-var)))
         (let ((,next (unpaused-node-next ,node-var)))
           (when ,next
             (setf ,f (node-funcons ,next)))))))

(declaim (inline node-next-funcons))
(defun node-next-funcons (node)
  (declare (type node node))
  (let ((next (unpaused-node-next node)))
    (when next (node-funcons next))))
        ...
        (case add-action
          (:before
           (unless (eq target *root-node*)
             ...
             ;; Note: the new empty group doesn't alter the previous funcons.
          ...
          (:after
           (unless (eq target *root-node*)
             ...
             (let ((prev (find-last-node target)))
               (setf (node-prev group) prev
                     (node-next group) (node-next prev)
                     (node-next prev) group))
             (setf (node-funcons group) (node-next-funcons group))
          ...
          (:head
           (when (group-p target)
             ...
             ;; This empty group is the first node of a group.
             (setf (node-funcons group) (node-funcons target))
             (nrt-msg info "new group ~D" id)
             group))
          (:tail
           (when (group-p target)
             ...
             (when (node-p (node-next group))
               (setf (node-prev (node-next group)) group)
               (update-next-funcons group t (paused-parent target)))
             (nrt-msg info "new group ~D" id)
             group)))))))
  ...
  (cond ((and (node-id target) (group-p target))
         ...
         (when (node-p (node-next item))
           ...
           (setf (cdr (node-funcons item))
                 (or (node-funcons target)
                     (if (and (paused-parent item)
                              (not (node-pause-p (node-next item))))
                         (node-funcons (node-next item))
                         (node-next-funcons item)))))
         (unless (node-pause-p target)
           (setf (node-funcons target) (node-funcons item))
           (update-previous-funcons target (paused-parent target)))
         (incf (int-hash-table-count *node-hash*))
         (start-with-fade-in item fade-time fade-curve)
         item)
        (t (setf (node-id item) nil))))

(defun add-node-to-empty-group (item target)
  (setf (node-next item) (node-next target)
        (node-next target) item
        (node-prev item) target
        (node-last target) item)
  (when (node-p (node-next item))
    (setf (node-prev (node-next item)) item)
    (setf (cdr (node-funcons item))
          (or (node-funcons target)
...
(defun add-node-to-tail (item target fade-time fade-curve)
  (if (and (node-id target) (group-p target))
      (let ((last (find-last-node target)))
        (setf (node-parent item) target)
        (cond ((node-p (node-last target))
               (setf (node-prev item) last
                     (node-next item) (node-next last)
                     (node-next last) item
                     (node-last target) item)
               (let ((paused (paused-parent item)))
                 (when (node-p (node-next item))
                   (setf (node-prev (node-next item)) item)
                   (update-next-funcons item nil paused))
                 (update-previous-funcons item paused)))
              (t (add-node-to-empty-group item target)))
        (incf (int-hash-table-count *node-hash*))
        (start-with-fade-in item fade-time fade-curve)
        item)
...
(defun add-node-before (item target fade-time fade-curve)
  (cond ((and (node-id target) (not (node-root-p target)))
         ...
         (let ((paused (paused-parent item)))
           (if (and paused (not (node-pause-p target)))
               (setf (cdr (node-funcons item)) (node-funcons target))
               (let ((next (unpaused-node-next item)))
                 (when next
                   (setf (cdr (node-funcons item)) (node-funcons next)))))
           (update-previous-funcons item paused))
         (incf (int-hash-table-count *node-hash*))
         (start-with-fade-in item fade-time fade-curve)
         item)
...
(defun add-node-after (item target fade-time fade-curve)
  (cond ((node-id target)
         ...
         (let ((prev (find-last-node target)))
           (setf (node-prev item) prev
                 (node-next item) (node-next prev)
                 (node-next prev) item))
...
(defun unlink-prev (n1 n2)
  ...
  ;; If N1 is paused, the previous funcons depends on N1 if the
  ;; previous node is paused. In this case the funcons is updated
  ;; from UNPAUSE.
  (unless (node-pause-p n1)
    (let ((prev (unpaused-node-prev n1)))
      (when prev
        (update-funcons prev (when (node-next n2) (node-next-funcons n2))
          :group-body ((update-previous-funcons prev))))
      (unless (eq prev (node-prev n1))
        ;; PREV N1 is paused.
        (update-funcons (node-prev n1) nil
          :group-body ((update-previous-funcons (node-prev n1) t)))))))
        ...
        (t (unlink-parent src)
           ...
           (let ((curr (find-last-node src)))
             ...
             (unless (node-pause-p src)
               (let ((paused (paused-parent src)))
                 (if (and paused (not (node-pause-p dest)))
                     ;; SRC, PREV SRC and DEST are paused by a parent.
                     (update-previous-funcons dest paused src)
                     (update-funcons curr (node-next-funcons curr)
                       :group-body ((update-previous-funcons curr nil src))))
                 (update-previous-funcons src paused))))
           src)))
...
(defun move-node-after (src dest)
  ...
  (cond ((eq dest *root-node*)
        ...
        (t (unlink-parent src)
           (unlink-group-neighbors src)
           (let* ((curr (find-last-node src))
                  (prev (find-last-node dest))
                  (next-p (node-p (node-next prev)))
                  (paused (paused-parent dest)))
             (setf (node-prev src) prev
                   (node-next curr) (node-next prev)
                   (node-next prev) src
                   (node-parent src) (node-parent dest))
             (when next-p
               (setf (node-prev (node-next curr)) curr))
             (when (eq dest (node-last (node-parent src)))
               (setf (node-last (node-parent src)) src))
             (unless (node-pause-p src)
               (if paused
                   (when next-p
                     (update-previous-funcons (node-next curr) t src))
                   (update-funcons curr (node-next-funcons curr)
                     :group-body ((update-previous-funcons curr nil src))))
               (update-previous-funcons src paused)))
           src)))
...
(defun move-node-head (src dest)
  ...
  (cond ((not (group-p dest))
        ...
        ((and (eq (node-parent src) dest)
              (eq (node-next dest) src))
         nil)
        ((eq (node-prev src) dest)
         (setf (node-parent src) dest
               (node-last dest) src))
        ...
        (t (unlink-parent src)
           (unlink-group-neighbors src)
           (let ((curr (find-last-node src))
                 (next-p (node-p (node-next dest)))
                 (paused (paused-parent dest)))
             (setf (node-next curr) (node-next dest)
                   (node-prev src) dest
                   (node-next dest) src
             ...
             (unless (node-pause-p src)
               (if paused
                   (when next-p
                     (update-previous-funcons (node-next curr) t dest))
                   (update-funcons curr (node-next-funcons curr)
                     :group-body ((update-previous-funcons curr nil dest))))
               (update-previous-funcons dest paused)))
           src)))
  ...
  (rt-eval (:return-value-p t)
    (incudine-optimize
      (unless (or (null-node-p obj) (node-pause-p obj))
        ...
        (unless (paused-parent obj)
          (if (node-root-p obj)
              (setf (node-funcons *root-node*) nil)
              (let ((prev (unpaused-node-prev obj)))
                (when prev
                  (update-funcons prev (if (group-p obj)
  ...
  (rt-eval (:return-value-p t)
    (incudine-optimize
      (when (and (node-pause-p obj) (not (null-node-p obj)))
        (let* ((root-p (node-root-p obj))
               (paused (unless root-p (paused-parent obj)))
               (next (unless paused (unpaused-node-next obj root-p))))
          (setf (node-pause-p obj) nil)
          (cond ((group-p obj)
                 (if next
                     ;; NEXT is after the last node. Here the funcons is
                     ;; linked with the last unpaused node within the
                     ;; group OBJ.
                     (update-previous-funcons next nil (node-next obj))
                     (let ((last (find-last-node obj)))
                       (unless (eq last obj)
                         (let ((last (if (and (not paused) (node-pause-p last))
                                         (unpaused-node-prev last)
                                         last)))
                           (when (and last (not (eq last obj)))
                             (update-funcons last nil
                               :group-body ((update-previous-funcons
                                              last paused (node-next obj)))))))))
                 (setf (node-funcons obj)
                       (unless paused (node-next-funcons obj))))
                (t (setf (cdr (node-funcons obj))
                         (when next (node-funcons next)))))
          (update-previous-funcons obj paused))
        obj))))
