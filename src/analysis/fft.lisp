...
(defun ifft-apply-window (obj &optional abuf)
  ...
  (let ((outbuf (ifft-output-buffer obj))
        ...
        (scale-factor (if abuf (abuffer-scale-factor abuf) (sample 1))))
    (cond ((and abuf (abuffer-normalized-p abuf))
          ...
          ((eq (ifft-window-function obj) #'rectangular-window)
           (apply-scaled-rectwin outbuf winsize scale-factor))
          (t (apply-scaled-window outbuf winbuf winsize scale-factor)))))
