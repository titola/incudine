...
(defun ifft-apply-window (obj abuf)
  ...
  (cond ((abuffer-normalized-p abuf)
        ...
        (t (apply-scaled-window (ifft-output-buffer obj)
                                (ifft-window-buffer obj)
                                (ifft-window-size obj)
                                (abuffer-scale-factor abuf)))))

(defun compute-ifft (obj abuffer &optional force-p)
  (declare (type ifft obj) (type abuffer abuffer) (type boolean force-p))
  (when (or force-p (< (analysis-time obj) (now)))
