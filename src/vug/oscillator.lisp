...
(define-vug buffer-play ((buffer buffer) rate start-pos (loop-p boolean) (done-action function))
  (with ((frames (buffer-frames buffer))
         (channels (buffer-channels buffer))
         (freq (* rate (/ *sample-rate* frames)
                  (the sample (buffer-sample-rate buffer))
                  *sample-duration*))
         (init (/ start-pos frames)))
    (declare (type non-negative-fixnum frames channels)
             (type sample freq init))
    (prog1 (buffer-read buffer (* (phasor freq init) frames)
