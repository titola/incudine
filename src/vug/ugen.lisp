...
(incudine.util:defun* compile-vug (name-or-vug (return-type t) force-p
          ...
          (t
           (let* ((arg-names (vug-args vug))
                  ...
                  (ugen (make-ugen :name name
                                   ...
                                   :arg-types types
                                   :defaults defaults
                                   :metadata-list (vug-metadata-list vug)
                                   :inline-callback fn)))
...
(defun get-ugen-specs (def)
  (multiple-value-bind (form doc)
    ...
    (do ((specs nil (cons (car l) specs))
         (l form (cdr l)))
        ((not (vug-spec-p l)) (values doc (nreverse specs) l)))))
...
(defun ugen-lambda-list (name)
  "Return the lambda list, the default values and the return type for
the UGEN NAME.

NAME can also be a UGEN structure."
  (declare (type (or symbol ugen) name))
  (let ((ug (if (ugen-p name) name (ugen name))))
    (if ug
        (values
          (mapcar #'list (ugen-args ug) (copy-tree (ugen-arg-types ug)))
          (copy-tree (ugen-defaults ug))
          (copy-tree (ugen-return-type ug)))
        (error 'incudine:incudine-undefined-ugen :name name))))

(defmethod metadata ((obj ugen) &optional type)
  (get-metadata obj type #'ugen-metadata-list))
