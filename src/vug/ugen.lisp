...
(defmacro generate-ugen-code (name arguments arg-names bindings ugen node obj)
...
(defmacro %compile-vug (name bindings return-type args arg-names ugen)
  ...
  (let ((doc (if (stringp (car body)) (car body))))
    (multiple-value-bind (args types) (arg-names-and-types lambda-list)
      `(%%codegen-debug ,name ,lambda-list generate-ugen-code
         ...
         (maybe-store-return-value ,return-type ,@(if doc (cdr body) body))))))
