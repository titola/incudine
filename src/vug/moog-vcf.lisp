  ...
  (with-samples (x0 y0 y1 stage0 stage1 stage2 stage3 tanh-0 tanh-1 tanh-2)
    (with-samples ((f (* freq *sample-duration*))
      ...
      (macrolet ((with-oversampling ((n) &body body)
        ...
        (with-oversampling (2)
          (setf x0 (- in (* res4 y0)))
          (incf stage0 (* tune (- (tanh (* x0 thermal)) tanh-0)))
          (setf tanh-0 (tanh (* stage0 thermal)))
          (incf stage1 (* tune (- tanh-0 tanh-1)))
          (setf tanh-1 (tanh (* stage1 thermal)))
          (incf stage2 (* tune (- tanh-1 tanh-2)))
          (setf tanh-2 (tanh (* stage2 thermal)))
          (incf stage3 (* tune (- tanh-1 (tanh (* stage3 thermal)))))
