    ...
    (let ((v *voi4*)
      ...
      (with-logger (*null-output*)
        (bounce-to-buffer (*buffer-test-c2* :frames 8)
          (voicer:trigger v tag)
          (at 1 #'voicer:release v tag)
          (at 2 (lambda ()
                  (push (voicer:empty-p v) acc)
                  (voicer:trigger v tag)))
          ;; The voice is not released because the TAG object is not the same.
          (at 3 #'voicer:release v (copy-list tag))
          (at 4 (lambda () (push (voicer:empty-p v) acc)))
          (at 5 #'voicer:release v tag)
          (at 6 (lambda () (push (voicer:empty-p v) acc)))))
      acc)
  (T NIL T))
...
(deftest voicer.12
    (let ((v *voi4*)
      ...
      (with-logger (*null-output*)
        (bounce-to-buffer (*buffer-test-c2* :frames 8)
          (dotimes (i 4) (voicer:trigger v i))
          (at 1 (lambda ()
                  (voicer:dovoices (tag v)
                    (push tag acc)
                    (voicer:release v tag)
                    (voicer:trigger v (+ tag 10)))))
          (at 2 (lambda ()
                  (voicer:dovoices (tag v)
                    (push tag acc)
                    (when (oddp tag)
                      ...
                      (voicer:trigger v (+ tag 10))))))
          (at 3 (lambda ()
                  (voicer:dovoices (tag v) (push tag acc))))))
      (nreverse acc))
  ( 0  1  2  3
   10 11 12 13
   10 12 21 23))
...
(deftest voicer.13
    (let ((v *voi4*)
      ...
      (with-logger (*null-output*)
        (bounce-to-buffer (*buffer-test-c2* :frames 8)
          (dotimes (i 8) (voicer:trigger v i))
          (at 1 (lambda ()
                  (voicer:dovoices (tag v)
                    (push tag acc)
                    (if (evenp tag) (voicer:release v tag)))))
          (at 2 (lambda ()
                  (voicer:dovoices (tag v)
                    (push tag acc)
                    (voicer:trigger v (+ tag 100)))))
          (at 3 (lambda ()
                  (voicer:dovoices (tag v) (push tag acc))))
          (at 4 (lambda ()
                  (push (let (A)
                          (voicer:dovoices (tag v (nreverse A))
                            (push tag A)
                            (voicer:release v tag)))
                        acc)))
          (at 5 (lambda ()
                  (push (let ((n 0))
                          (or (voicer:dovoices (tag v n) (incf n)) n))
                        acc)))))
      (nreverse acc))
  (0 1 2 3 4 5 6 7
   1 3 5 7
